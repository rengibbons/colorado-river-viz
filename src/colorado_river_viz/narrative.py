...
def describe_supply_vs_compact(
    ...
    e.g. "Since 2000, the river has averaged 12.0 MAF a year, 27% short of
    ...
    return (
        f"Since {since_year}, the river has averaged {mean_maf:.1f} MAF a year, "
...
def describe_runoff_year(row: pd.Series[float]) -> str:
    """A water year's runoff-efficiency headline, e.g. "2026 produced 1.1 MAF
    of spring runoff from 9.1 in of peak snowpack, only 35% of the normal
    efficiency."."""
    return (
...
def describe_bridge_fit(fit: BridgeFit) -> str:
    """The natural-flow bridge's fit statistics, e.g. "Fit on 1964-2020
    (n=57): natural = 0.62 + 1.02 x Powell unregulated inflow (MAF); residual
    SD 0.85 MAF."."""
    return (
        f"Fit on {fit.first_year}-{fit.last_year} (n={fit.n}): "
